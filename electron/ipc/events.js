 ...
 */
export function registerSyncIPCHandlers(syncQueries) {
    ...
    ipcMain.handle(IPC_EVENTS.GET_MESSAGES, async (event, { chatId, limit, offset }) => {
        ...
        try {
            ...
            const messages = await syncQueries.getMessagesForChat(chatId, limit || 50, offset || 0);
    ...
    ipcMain.handle(IPC_EVENTS.SEND_MESSAGE, async (event, { chatId, content, recipient }) => {
        try {
            if (!chatId || !content) {
